...
router.post("/retail/gastos", (req, res) => {
  const { nombre, proveedor, monto, tipo } = req.body;
  if (!nombre) return res.status(400).json({ error: "Falta nombre" });
  crearGasto(nombre.trim(), proveedor||null, monto, 1, tipo||'salida');
  res.status(201).json({ ok: true });
});
router.patch("/retail/gastos/:id", (req, res) => {
  actualizarGasto(req.params.id, req.body.nombre, req.body.proveedor, req.body.monto, req.body.kg_bulto||1, req.body.tipo||'salida');
  res.json({ ok: true });
});
