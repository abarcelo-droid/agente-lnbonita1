...
// Snapshot del stock teórico al momento del conteo. Necesario para reconstruir
// el delta histórico — el teórico cambia con cada despacho/ingreso, así que
// sin este snapshot la diferencia "fecha X" es imposible de reproducir después.
try { db.exec("ALTER TABLE ifco_stocks_reales ADD COLUMN teorico_snapshot INTEGER"); } catch(_){}
...
// Usa el approach robusto: preservar el SQL original literal y solo modificar la columna,
// para no perder defaults con expresiones como `(datetime('now','localtime'))` ni FKs.
try {
  ...
  if (provCol && provCol.notnull === 1) {
    ...
    const sqlOriginal = tblRow && tblRow.sql ? String(tblRow.sql) : '';
    if (!sqlOriginal) {
      console.warn('[IFCO] No se encontró el SQL de ifco_recepciones_proveedor');
    } else {
      // Reemplazar la definición de proveedor_id quitando el NOT NULL.
      // Tolera distintos tipos (INTEGER, TEXT, etc.) y orden de modificadores.
      const sqlNew = sqlOriginal.replace(
...
// ════════════════════════════════════════════════════════════════════════════
// ENDPOINT TEMPORAL DE DEBUG — REMOVER DESPUÉS DEL DIAGNÓSTICO
// ════════════════════════════════════════════════════════════════════════════
// Devuelve el SQL real de las tablas IFCO en producción + cuenta de filas.
// Sirve para diagnosticar discrepancias entre el código y el schema real de la DB.
router.get('/_debug/schema', function(req, res) {
  try {
    const tablas = [
      'ifco_movimientos',
      'ifco_envios_proveedor',
      'ifco_recepciones_proveedor',
      'ifco_remitos_super',
      'ifco_stocks_reales',
      'ifco_talonarios',
      'ifco_talonarios_log'
    ...
    for (const t of tablas) {
      try {
        const tblRow = db.prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name=?").get(t);
        const cols = db.prepare("PRAGMA table_info(" + t + ")").all();
        const idxs = db.prepare("SELECT name, sql FROM sqlite_master WHERE type='index' AND tbl_name=?").all(t);
        const count = db.prepare("SELECT COUNT(*) AS c FROM " + t).get();
        out[t] = {
          sql: tblRow ? tblRow.sql : null,
          columnas: cols.map(c => ({ name: c.name, type: c.type, notnull: c.notnull, dflt_value: c.dflt_value, pk: c.pk })),
          indices: idxs.map(i => i.name),
          filas: count.c
        };
      } catch(e) {
...
router.get('/stocks-reales', function(req, res) {
  ...
  res.json({
    ...
    // Suma de diferencias negativas del último conteo de cada depósito.
    // Indicador rápido de cuántos cajones "están perdidos" hoy.
    perdida_actual_total: items.reduce(function(acc, it) {
      return acc + ((it.diferencia != null && it.diferencia < 0) ? it.diferencia : 0);
    }, 0),
    items: items
  });
});
...
router.get('/stocks-reales/historico', function(req, res) {
  ...
  // Para cada row, calcular diferencia (real - teorico_snapshot) y delta vs el conteo
  // inmediatamente anterior (rows está en orden DESC, así que el "anterior cronológico"
  // es el siguiente en el array).
  for (let i = 0; i < rows.length; i++) {
    const cur = rows[i];
    cur.diferencia = (cur.teorico_snapshot != null) ? (cur.cantidad - cur.teorico_snapshot) : null;
    if (i < rows.length - 1) {
      const prev = rows[i + 1];
      const difPrev = (prev.teorico_snapshot != null) ? (prev.cantidad - prev.teorico_snapshot) : null;
      cur.delta_vs_anterior = (cur.diferencia != null && difPrev != null)
        ? (cur.diferencia - difPrev) : null;
    } else {
      cur.delta_vs_anterior = null;
    }
  }
  res.json(rows);
});
...
router.post('/stocks-reales', express.json(), function(req, res) {
  try {
    ...
    const provIdNum = d.deposito_tipo === 'proveedor' ? parseInt(d.proveedor_id) : null;
    // Snapshot del teórico al momento del conteo. Sin esto, la diferencia
    // histórica no se puede reconstruir (el teórico cambia con cada movimiento).
    const teoSnap = _stockTeoricoDeposito(d.deposito_tipo, provIdNum);
    const r = db.prepare(`
      INSERT INTO ifco_stocks_reales (deposito_tipo, proveedor_id, cantidad, fecha, notas, usuario_id, teorico_snapshot)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(d.deposito_tipo, provIdNum, cant, fecha, d.notas || null,
           (req.user && req.user.id) || null, teoSnap);
    res.json({ id: r.lastInsertRowid, teorico_snapshot: teoSnap, diferencia: cant - teoSnap });
  } catch(e) {
